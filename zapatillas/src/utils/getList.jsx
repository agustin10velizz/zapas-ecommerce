const Productos = [
    {   
        id: "1",
        Titulo:"Vans",
         Color: "Negro",
          Talles:"39, 40, 41",
           Precio:"$2500",
            Imagen: "https://media.revistagq.com/photos/5fc60d39be088082688e9834/master/w_1600%2Cc_limit/1973657270-01-01.jpg",
             categoryId: 1,
    },
    {
        id:"2",
        Titulo:"Converse",
        Color: "Blanco",
         Talles:"39, 40, 41",
          Precio:"$2300",
           Imagen: "https://www.converse.com/dw/image/v2/BJJF_PRD/on/demandware.static/-/Sites-cnv-master-catalog-we/default/dwb5e2dce7/images/a_107/M7652_A_107X1.jpg?sw=964", 
           categoryId: 2,
        },
    {
        id:"3",
        Titulo:"Adidas",
        Color: "Blanco",
         Talles:"39, 40, 41",
          Precio:"$3500",
          Imagen: "https://assets.adidas.com/images/w_600,f_auto,q_auto/7ed0855435194229a525aad6009a0497_9366/Zapatillas_Superstar_Blanco_EG4958_01_standard.jpg",
          categoryId: 3,

        },
    {
        id:"4",
        Titulo:"Vans",
         Color: "Rojo",
          Talles:"39, 40, 41",
           Precio:"$2500",
            Imagen: "https://images.vans.com/is/image/VansEU/VN0A38G1VG4-HERO",
            categoryId: 1,

        },
    {
        id:"5",
        Titulo:"Converse",
        Color: "Negro",
         Talles:"39, 40, 41",
          Precio:"$2300",
           Imagen: "https://http2.mlstatic.com/D_NQ_NP_847516-MLA48678276086_122021-O.webp",
           categoryId: 2,

        },
    {
        id:"6",
        Titulo:"Adidas",
        Color: "Negro",
         Talles:"39, 40, 41",
          Precio:"$3500",
           Imagen: "https://assets.adidas.com/images/w_600,f_auto,q_auto/a8080c6d00924b9f8673a9c300cf22f9_9366/Zapatillas_U_Path_Run_Negro_G27636_01_standard.jpg",
           categoryId: 3,

        },
        {
            id: "7",
            Titulo:"Vans",
             Color: "Azul",
              Talles:"39, 40, 41",
               Precio:"$2500",
                Imagen: "https://www.moov.com.ar/on/demandware.static/-/Sites-dabra-catalog/default/dwff03ddbe/products/VA_VN000D3HNVY/VA_VN000D3HNVY-1.JPG",
                 categoryId: 1, 
        },
        {
            id:"8",
             Titulo:"Converse",
              Color: "Marrones",
               Talles:"39, 40, 41",
                 Precio:"$2300",
                   Imagen: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTW9wEPJYxvMNuniBvBN-lPVwgcEGSMEp2gwQ&usqp=CAU", 
                    categoryId: 2, 
        },
        {
            id:"9",
             Titulo:"Adidas",
               Color: "Verde",
                Talles:"39, 40, 41",
                  Precio:"$3500",
                   Imagen: "https://assets.adidas.com/images/w_600,f_auto,q_auto/e95b7e759132479abbe2abeb00ba83dc_9366/Zapatillas_Runfalcon_Verde_G28729_01_standard.jpg",
                    categoryId: 3,
        }
];

/*
const getList = () => {
    return new Promise ( (resolve, reject) =>{
        setTimeout ( () =>{
            resolve (Productos)
        }, 2000);
    })
}*/



export default Productos;